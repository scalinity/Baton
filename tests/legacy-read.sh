...
# Existing Git identity is used. The temporary test repository disables hooks/signing per call.
...
for sc in "$here"/scenarios/*/; do
  ...
  name=$(basename "$sc")
  case "$name" in
    derive-ladder|log-without-lock) continue ;; # stronger contract-2 assertions are in foundation.sh
    plan-*|derive-*|session-name|log-torn-line) ;;
    *) continue ;; # historical v1 mutation snapshots are superseded by contract-2 behavior tests
  esac
  count=$((count + 1))
  tmp=$tmproot/$name
  ...
  ( cd "$tmp/Fixture" \
    ...
    && git -c core.hooksPath=/dev/null -c commit.gpgsign=false commit -q -m "fixture" )
  ...
  offmain=$(git -C "$tmp/Fixture" commit-tree "$commit^{tree}" -p "$commit" -m off-main)
