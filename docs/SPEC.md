...
| REQ-ESC-05 | A parked lane unparks in exactly three ways, each a `resolution` event: `baton answer` delivers a ruling, or accepts a hand-back ruling for a dispatch-failed park with no session; a live question was answered in place, which the row leaving `waiting` or the session's own later artifact shows; a person edits the plan rows or the brief a park answers to and the next tick re-reads it. A resolution ends the park: the rule that raised it does not raise it again for the same ending — after an edit it takes the next step, after a delivered ruling it waits for the session's next ending; a sessionless dispatch-failed hand-back permits normal candidate admission again. | Derivation 2 over fixtures. | Where an escalation goes §6 |
