...
| REQ-DISPATCH-03 | Every milestone runs in `../<Project>-M<nn>` on branch `m<nn>`, created by `git -C <path> worktree add ../<Project>-M<nn> -b m<nn> main` — from `main` explicitly — and reused on redispatch with the reuse and its commit logged. The session removes it at close-out; the tick prunes a leftover only when `Status` reads `done`, no live session belongs to it, and the archived artifact's `merged_as` is an ancestor of `main` (`worktree_pruned`), with `git worktree remove` and never `--force`; a leftover whose directory is already gone has its registration dropped under the same guards (D-075). | Fixture repo; prune refused on a `merge-failed` lane, on a live session, and on a `Status` that is not `done`. | Dispatching more than one at once §5 |
