...
One person, one Mac, Claude Code 2.1.268 at `/Users/danny/.local/bin/claude`. Projects under
...
| REQ-TICK-03 | An atomic `mkdir` lock serialises a launchd firing against a hand run; every verb runs under it and the log is written only under it. A stale lock is reported first by `status` and is the `baton-unhealthy` project-scope escalation. | A held lock makes a second tick exit without acting; the age of a stale lock is printed. | Relay or conductor §4, §6; Where an escalation goes §5 |
...
| REQ-TICK-08 | The tick reaches the CLI by absolute path (`/Users/danny/.local/bin/claude`), sets `LC_ALL` before parsing `backgrounded · <id>` (the separator is two bytes and launchd sets no `LANG`), and every outside thing — the claude binary, the clock, caffeinate — is reached through one variable so a test can point it at a shim. | The three seams are named in `docs/ARCHITECTURE.md` §7 and every test uses them. | Relay or conductor §5; Watching a dispatched session |
| REQ-TICK-09 | The tick is a launchd job whose executable is `/Users/danny/.baton/bin/sh`, a copy of `/bin/sh` granted Full Disk Access, because a launchd job run by `/bin/sh` cannot execute a script under `~/Documents`, read a file there, or run git there. Nothing under `~/Documents` is executed by the job; the installed relay lives under `~/.baton/bin/`. | Prototype run C: `Operation not permitted` on `cat` and `git` under `~/Documents`; items 36–38 prove the granted copy. | Dispatching more than one at once §2 |
...
| REQ-STOP-12 | Declared stops: `unfinished` → redispatch attempt n+1, not a failure; a second consecutive `unfinished` escalates with both splits. `blocked` with `blocked_by` → leaves the in-flight set, silent while the blocker is in flight or eligible, redispatched when the plan shows it `done`, otherwise escalates; a distant `wait_for` notifies once the same way. `blocked` naming nothing, `merge-failed`, `other` → escalate with the detail. `merge-failed` and `main-broken` resolve by a ruling that resumes the session to finish the close-out from step (c); the plan-file edit is the fallback when the session is gone. `main-broken` parks the project; in-flight lanes run on. | One fixture per reason. | What stops a session, and what happens next §7, §8; amended by Dispatching more than one at once |
...
| REQ-STOP-14 | Takeover: a lane is taken over exactly while its transcript's newest typed record (`type == "user"`, `promptSource == "typed"`, not `isMeta`, not `isCompactSummary`) is one Baton did not send, matched by hash against every prompt sidecar of that `(project, milestone)`. Baton then stops acting on the lane — no resume, redispatch, ruling or ladder step — still consumes an artifact that lands, still counts the lane against the cap, notifies once on the Mac, and restarts the long-running clock. Release: a `resume` makes Baton's prompt the newest typed record, or the row goes idle and a handover for the milestone has been consumed. Not a park. | Live item 45 (hash normalisation) passes before the rule is trusted; a fixture transcript with a foreign typed record reads as taken over and a later Baton prompt releases it. | The dispatch log §4; Watching a dispatched session |
...
| REQ-ESC-02 | Both reach the Mac as a `display notification` through `osascript` plus a log event. Nothing reaches the phone automatically: a self-sent iMessage raises no iOS alert, so the phone is reached only by Remote Control on a `Remote: yes` dispatch. | Prototype items 32, 33 and the addendum. | Where an escalation goes §5; Watching a dispatched session; The dispatch log §2 footer |
...
| REQ-ESC-05 | A parked lane unparks in exactly three ways, each a `resolution` event: `baton answer` delivers a ruling; the row leaves `waiting` because a live question was answered in place; a person edits the plan file or a brief and the next tick re-reads it. | Derivation 2 over fixtures. | Where an escalation goes §6 |
| REQ-ESC-06 | `baton answer <milestone> <ruling \| option number>` resolves against parked lanes across all projects: one match acts; more than one refuses and prints `<project>/<milestone>` candidates, which is also the accepted long form; a milestone not parked is refused ("nothing is waiting on M19"). An option number expands to the option's text before delivery. Delivery is stop-then-flagless-resume with the ruling label in `docs/ARCHITECTURE.md` §4. `baton answer <milestone> "continue"` is also the hand-back after a takeover. | Fixtures for one, two and zero matches. | Where an escalation goes §7; The dispatch log §4 |
...
| REQ-ESC-08 | Remote Control is never automatic: only a plan-file `Remote: yes` dispatches with it, because a connected session's transcript is stored on Anthropic's servers. For such a session the row is not a park detector; parks are artifact-borne, and a prompt nobody answers surfaces as a stall. | Fixture row `working/idle` for a remote lane is not read as a park. | Where an escalation goes §5; Dispatching more than one at once §7 |
| REQ-ESC-09 | `baton allow <milestone> '<rule>'` writes exactly the rule given to the project's `permissions.json` and the dispatched settings file in place, logs a `widening` event with the rule and the milestone that earned it, and refuses to write an `ask` rule. Baton never composes a rule. `--resume` stops and flaglessly resumes the session so the re-issued command passes. | Prototype item 35. | Where an escalation goes §4; amended by The dispatch log |
...
| ID | Requirement | Acceptance | Decided by |
|---|---|---|---|
| REQ-DISPATCH-01 | For each eligible milestone the disposition in force is the one in the newest archived `complete` handover of that project that lists it: `run` → candidate; `wait` → honoured while every `wait_for` is in flight, eligible or done, dispatched the moment all read `done`; `held` on a gate the plan shows cleared → dispatched with a `plan_override`; `run` on a milestone the plan holds → withheld with a `plan_override`. Exactly two things escalate, lane scope, by name: a `run` the plan makes ineligible (`disagreement`) and a plan-eligible milestone no handover lists (`omitted`). | Six fixtures, one per branch. | Dispatching more than one at once §3 step 6 |
| REQ-DISPATCH-02 | Then the holds and the cap: drop candidates on a held model; drop `fable` candidates while the freshest status file's `seven_day.used_percentage` ≥ `fableReserve`; count in flight as every logged session with a live row plus every parked lane holding a live prompt (stopped `asking` sessions do not count); dispatch while below `cap`, in the handover's `eligible[]` order within a project, then the project with fewer in flight, then plan row order. | Cap fixture with three candidates dispatches two in the stated order. | Dispatching more than one at once §3 step 7, §6, §7 |
| REQ-DISPATCH-03 | Every milestone runs in `../<Project>-M<nn>` on branch `m<nn>`, created by `git -C <path> worktree add ../<Project>-M<nn> -b m<nn> main` — from `main` explicitly — and reused on redispatch with the reuse and its commit logged. The session removes it at close-out; the tick prunes a leftover only when `Status` reads `done`, no live session belongs to it, and the archived artifact's `merged_as` is an ancestor of `main` (`worktree_pruned`). | Fixture repo; prune refused on a `merge-failed` lane. | Dispatching more than one at once §5 |
...
| REQ-PERM-03 | The dispatched settings file carries `permissions.defaultMode` (documentation only), the project's `allow` and `deny` copied, no `ask` rules, and three hooks — `Stop` (the gate), `StopFailure`, `statusLine` — each command carrying `BATON_PROJECT` and `BATON_MILESTONE` as environment on its command line and pointing at the installed relay under `~/.baton/bin/`. A hand-started session passing the same file behaves as a dispatched one. | Composed file equals the fixture shape in `docs/ARCHITECTURE.md` §3. | Dispatching more than one at once §4; What stops a session §2a |
...
| REQ-LOG-07 | The archive, not the log, says what was consumed; the `consumed` event's `archive` field holds the archived filename verbatim and a reader joins on the field. The status feed (`~/.baton/status/<session_id>.json`, overwritten per turn) and the tick marker are beside the log and never rotated. | Derivation 4. | The dispatch log §5, §7 |
...
| ID | Requirement | Acceptance | Decided by |
|---|---|---|---|
| REQ-VERB-01 | One script, `baton`, with the verbs `tick`, `answer`, `status`, `plan`, `dispatch`, `allow`; every verb takes the lock; `status` and `plan` have no side effects. | `baton` with no verb prints the six. | Relay or conductor §4, §6 |
...
| REQ-SETUP-05 | **The installed relay.** `sh install.sh` from the checkout copies `bin/`, `lib/` and `hooks/` to `~/.baton/bin/` and creates `~/.baton/{inbox,archive,rejected,status,settings,prompts,projects}` and `config.json` if absent. A merge on `main` changes nothing until it is run, so a milestone cannot break the tick that dispatched it. | Idempotent; a second run changes nothing. | Baton's plan and its M01 prompt (D-018) |
| REQ-SETUP-06 | **Remote Control** needs `DISABLE_TELEMETRY` unset in the launching environment (commented out at `~/.zshrc:10` on 2026-09-11; processes started earlier still carry it) and a registered mobile app; `claude doctor` confirms eligibility. Needed only for a `Remote: yes` dispatch. | `claude doctor` reports Remote Control available. | Reaching you when you're not at the Mac; Watching a dispatched session |
...
7. Reads a transcript for anything but its modification time and its typed records' hashes.
...
| INV-03 | An artifact's `merged_as` is verified as an ancestor of `main` before a `complete` handover is acted on. | Fixture with a lying artifact ends in `rejected/` |
...
- The phone hears nothing unless a milestone is `Remote: yes`; a self-sent iMessage raises no alert.
...
Nothing under `~/.baton/` is deleted by any verb except a leftover worktree under REQ-DISPATCH-03.
...
| 39–40 | `--settings` and `--model` given at an idle `--remote-control` start are restored by the flagless resume; whether `remoteControlAtStartup` collapses the two-step dispatch into one. | M07 |
