...
8. **Dispatch**, per candidate:
   ...
   - Read the kickoff prompt from the brief on `main` and replace part 2 whole with the slot line
     ...
     ends the pass rather than being replaced by the stale one (D-175).
   - Resolve the effort: the `Effort` cell, else the Size the brief declares in its `## 1.` section
     ...
     off (D-174). The planning lane takes model and effort from `config.json` instead (§6.4).
...
D-176, because the basename it named before is the milestone's own id under M09's managed root and
...
first reads "Nothing else is in flight", which at the moment it starts is true (D-175).
