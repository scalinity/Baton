...
| `notification` | `class`, `key`, and the class's own fields; `channel` — `["record"]` when `record_notification` wrote it and raised nothing, **absent** when `notification_write` delivered it. The asymmetry with `escalation`, whose field is always written, is deliberate and priced: nothing decides on the field, and backfilling `["notification"]` onto the delivered case would move forty-nine events across thirty-six frozen expectations without changing a decision (D-190). A `gap` also carries `disposition` and `host` (REQ-ESC-14) | the once-only rule, which a recorded event spends exactly as a delivered one does; `status`'s recorded-gap line | per `(project, milestone, attempt, class)`, with the class's extra term where it has one |
...
15. **The gap**: the as-of minus `~/.baton/last-tick`, the as-of defaulting to `now`. Reported as a `notification` with class `gap`
    ...
    **The tick measures against its own start**, `tr_now`, when it reads the gap after its lanes;
    ...
    check reported the length of the check as an outage. The gap means "Baton was not running", not
    "the marker is old", and the tick's own start is where those are the same sentence. The closed-lane
    test stays on the marker, since a lane that closed while the tick worked is still one the gap
    would have covered.
