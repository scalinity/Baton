...
Twenty-one kinds. Fields listed are those beyond the envelope.
...
- **The gap report is a derivation, not an event.** The gap is the reading instant minus
  `~/.baton/last-tick` — the tick's own start when the tick reads it, `now` for everyone else (D-153);
...
8. **The last tick.** `~/.baton/last-tick`, read as a file, not derived from the log. Its
   ...
   the length of the work that tick has done (D-153).
...
15. **The gap**: the as-of minus `~/.baton/last-tick`, the as-of defaulting to `now`. Reported as a `notification` with class `gap`
    ...
    **The tick measures against its own start**, `tr_now`, when it reads the gap after its lanes;
    ...
    could not list the rows, which returns before step 2 and so has spent nothing (D-153). Step 2 runs a target project's standing check under the tick lock (D-148), which is
