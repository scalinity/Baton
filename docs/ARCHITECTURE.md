...
| M07-c | `notify/Baton.applescript`, compiled by `install.sh` into `~/.baton/bin/Baton.app` (`com.baton.notify`, `LSUIElement`, Claude's icon when `BATON_CLAUDE_ICON` — default `/Applications/Claude.app/Contents/Resources/electron.icns` — exists, signed ad hoc, rebuilt only when the source or the icon differs) (D-092): a launch posts every file in `~/.baton/notify/spool/` and writes the newest one's target to `~/.baton/notify/target`; a launch with nothing spooled is a click, which clears Baton's delivered notifications and opens the target, or `claude://code/needs-input` when it is empty. `lib/notify.sh`: `notify <title> <body> [<session>]` (spools and runs `$BATON_OPEN -g Baton.app` when the applet is installed, `osascript` otherwise or when the launch fails); `notify_line <string>` (the capped line, unescaped); `session_url <session>` (the newest claude.ai session URL among the transcript's `remote_session_change` attachments, else the job state's `bridgeSessionId` as one, else empty; D-093). `escalate` and `notification_write` pass the event's session. The `question` verb reads `answer it in place in Claude.app, or baton answer <milestone> "<ruling>"`. Seams `BATON_OPEN` and `BATON_JOBS`; `tests/shim/open`; scenarios `notify-session-url`, `notify-job-state`, `notify-no-session-url`, `notify-applet-missing`, `notify-open-fails`, `install-no-claude-icon`, and the `install` scenario's applet checks | `notify`, `notification_write`, `escalate`, `message_render`, `escalation_verb`, `transcript_of`, `session_id_ok`, `now_epoch`; M07-b |
