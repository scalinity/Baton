...
| M14 | Budget-aware pacing | M13 | opus | medium | | done | Budget exhaustion produces a visible pause and resume time; work resumes automatically when the window rolls | No |
...
brief on `main`. The AI route is **recorded and not scheduled** (D-183): a row would put work nobody
