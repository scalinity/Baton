#!/bin/sh
# lib/log.sh — the dispatch log's one writer, the envelope, the attempt derivation, the prompt
# sidecar and its hash. Nothing else in Baton writes log.jsonl (INV-02).
set -eu
...
# An empty envelope argument is absent from the line, never null. Refuses without the lock.
log_event() {
...
attempt_of() {
  if [ ! -f "$BATON_HOME/log.jsonl" ]; then echo 0; return 0; fi
  jq -s --arg p "$1" --arg m "$2" \
    '[ .[] | select(.kind == "dispatch" and .project == $p and .milestone == $m) ] | length' \
    "$BATON_HOME/log.jsonl"
}
...
# the CLI stores the argument as given). Proved on the prototype's pair (M01, item 45).
prompt_normalise() {
...
# sidecar_write <session> <text>: writes prompts/<session>/<n>.txt, n = existing files + 1,
# and prints "<path> <sha256>" of the normalised text.
sidecar_write() {
  sw_dir=$BATON_HOME/prompts/$1
  mkdir -p "$sw_dir"
  sw_n=$(ls "$sw_dir" | wc -l | tr -d ' ')
  sw_n=$((sw_n + 1))
...
widenings_json() {
  ...
  jq -sc --arg p "$1" '[ .[] | select(.kind == "widening" and .project == $p) ] | reverse' "$BATON_HOME/log.jsonl"
}
